import unittest
from src.satisfaction_degree import weighted_signed_distance, find_extreme_depth
...
class TestDistanceFunctions(unittest.TestCase):
    ...
    def test_find_extreme_state_inside(self):
        dov = [[0, 1, 2, 3], [2, 5, 7, 8]]
        max_values = [4, 8]
        state, distance = find_extreme_depth(dov, max_values, is_inside=True)
    ...
    def test_find_extreme_state_inside_ambiguous(self):
        ...
        max_values = [8, 8]
        _, distance = find_extreme_depth(dov, max_values, is_inside=True)
        self.assertEqual(distance, 1 * (1/9))

    def test_find_extreme_state_outside_ambiguous(self):
        dov = [[1, 5], [0, 1, 2, 7, 9]]
        max_values = [5, 9]
        state, distance = find_extreme_depth(dov, max_values, is_inside=False)
